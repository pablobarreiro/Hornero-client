...
export const userRegister = createAsyncThunk("USER_REGISTER", (data) => {
   return axios.post(`${process.env.REACT_APP_CORS_URL}api/users/register`, data)
        .then(user => user.data)
});

export const userUpdate = createAsyncThunk("USER_UPDATE", (data) => {
    return axios.put(`${process.env.REACT_APP_CORS_URL}api/users/${data[1]}`, data[0])
        .then(user => user.data)
});

export const getUser = createAsyncThunk("GET_USER", () => {
    const userId = JSON.parse(localStorage.getItem('user')).user._id
    return axios.get(`${process.env.REACT_APP_CORS_URL}api/users/${userId}`)
        .then(user => user.data)
})

export const userLogin = createAsyncThunk("USER_LOGIN", (data) => {
    return axios.post(`${process.env.REACT_APP_CORS_URL}api/users/login`, data)
        .then(user => {
...
export const userLogout = createAsyncThunk("USER_LOGOUT", () => {
    return axios.post(`${process.env.REACT_APP_CORS_URL}api/users/logout`)
        .then(user => user.data)
});
